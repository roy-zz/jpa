...
수정할 때 파라미터로 넘어온 item 엔티티 인스턴스는 준영속 상태다. 따라서 영속성 컨텍스트의 지원을 받을 수 없고 데이터를 수정해도
변경 감지 기능은 동작하지 않는다.

**준영속 엔티티**는 영속성 컨텍스트가 더 이상 관리하지 않는 엔티티를 말한다.
수정을 시도하려는 Book 객체다. Book 객체는 이미 DB에 한번 저장되어서 식별자가 존재한다.
...
3. 조회한 영속성 엔티티(mergeMember)에 member 엔티티의 값을 채워 넣는다. member 엔티티의 모든 값을 mergeMember에 밀어 넣는다.
이때 mergeMember의 회원1이라는 이름이 "회원명변경"으로 바뀐다.
4. 영속 상태인 mergeMerge를 반환한다.

병합 시 동작 방식을 간단히 정리
1. 준영속 엔티티의 식별자 값으로 영속 엔티티를 조회한다.
2. 영속 엔티티의 준영속 엔티티의 값으로 모두 교체한다.(병합한다.)
3. 트랜잭션 커밋 시점에 감지 기능이 동작해서 데이터베이스에 UPDATE SQL이 실행된다.
...
이미 한번 영속화 되었던 엔티티로 판단해서 merge()로 수정(병합)한다. 결국 여기서의 저장(save)이라는 의미는
신규 데이터를 저장하는 것뿐만 아니라 변경된 데이터의 저장이라는 의미도 포함한다. 이렇게 함으로써
이 메서드를 사용하는 클라이언트는 저장과 수정을 구분하지 않아도 되므로 클라이언트의 로직이 단순해진다.

여기서 사용하는 수정(병합)은 준영속 상태의 엔티티를 수정할 때 사용한다. 영속 상태의 엔티티는 변경 감지(dirty checking)
...
병합을 사용하면서 이 문제를 해결하려면 변경 폼 화면에서 모든 데이터를 항상 유지해야한다.
...
트랜잭션이 있는 서비스 계층에서 식별자와 변경할 데이터를 명확하게 전달한다.
